function DMDT = Distillation_column_last_version(~,S,U)

% U(1) = 1.323; % Reflux Ratio
% U(2) = 2.323; % BoilUp Ratio
% U(3) = 50;  % Distillate Product Flowrate
% U(4) = 50;  % Bottom Product Flowrate
% U(5) = 100;  % Feed Flowrate
% U(6) = 0.5; % Mole Fraction Of feed
% U(7) = 1; % Saturated Liquid 
...
for h=1:61
    ...
    if 1 < h && h ~= 23 && h < 61
        rho_l(h) = ((X(h) * V_c1 * Z_c1^((1-((T(h)+273)/425.1))^(2/7))) +...
        ((1-X(h)) * V_c2 * Z_c2^((1-((T(h)+273)/469.7))^(2/7))))^(-1); % [kmol/m^3]
        ...
        T(h) = X(h)*((B_1_ANTOEINE/(A_1_ANTOEINE-log(p(h)/1000))-C_1_ANTOEINE)) +...
        (1 - X(h))*((B_2_ANTOEINE/(A_2_ANTOEINE-log(p(h)/1000))-C_2_ANTOEINE)); % celcuis
        ...
        alpha(h) = (gamma1*exp(A_1_ANTOEINE-(B_1_ANTOEINE)/(T(h)+C_1_ANTOEINE)))/...
        (gamma2*exp(A_2_ANTOEINE-(B_2_ANTOEINE)/(C_2_ANTOEINE+T(h))));
    ...
    if h==23
        ...
        T(h) = X(h)*((B_1_ANTOEINE/(A_1_ANTOEINE-log(p(h)/1000))-C_1_ANTOEINE)) +...
        (1 - X(h))*((B_2_ANTOEINE/(A_2_ANTOEINE-log(p(h)/1000))-C_2_ANTOEINE));
        ...
        gamma1 = exp((1-X(h))^2*(tav21*(G21/(X(h)+(1-X(h))*G21)^2)+...
        (G12*tav12)/(1-X(h)+X(h)*G12)^2));
    ...
    if h==1
        ...
        rho_l(h) = (0.99*V_c1*Z_c1^((1-(322/425.1))^(2/7))+...
        (1-0.99)*V_c2*Z_c2^((1-(322/469.7)))^(2/7))^(-1);
        ...
        gamma2 = exp(X(h))^2*(tav12*(G12/((1-X(h))+(X(h)*G12))^2)+...
        (G21*tav21)/(X(h)+(1-X(h))*G21)^2);
...
% Control Valve Constant For Bottom Product
valve_constant1=(50/rho_l(1))/(0.0865*((0.6807/(rho_l(1)/1000))^0.5))*3600; 

% Control Valve Constant For Distillate Product
...
%%  Reboiler
    if i==61
        ...
        M_hold(i) = L(i-1) - V(i) - rho_l(61)* valve_constant2*0.0865*(U(4)-0.5)*...
        (1/3600)*sqrt(2*(S(61)/(rho_l(61)*pi*0.81)*9.81*(rho_l(61)*AVMW)/1000-4.5)/(rho_l(i)*AVMW/1000))^(0.5);
        
        MXdt_hold_component(i) = X(i-1) * L(i-1) - V(i) * Y(i) + X(i)*rho_l(61)* valve_constant2*...
        0.0865*(U(4)-0.5)*(1/3600)*sqrt((S(61)/(rho_l(61)*pi*0.81)*9.81*(rho_l(61)*AVMW)/1000-4.5)/(rho_l(i)*AVMW/1000))^(0.5);
...
%%   Condenser
    if i==1
        ...
        M_hold(i) = V(i+1)-L(i)-rho_l(1)*( valve_constant1*0.0865*(U(3)-0.5)*(1/3600)*...
        ((S(1)/(rho_l(1)*pi*0.81)*9.81*rho_l(1)*AVMW)/1000-4.5)/(rho_l(i)*AVMW/1000))^(0.5);

        MXdt_hold_component(i) = V(i+1)*Y(i+1)-L(i)*X(1) - rho_l(1)*(valve_constant1*0.0865*(U(3)-0.5)* ...
        (1/3600)*((S(1)/(rho_l(i)*pi*0.81)*9.81*rho_l(1)*AVMW)/1000-4.5)/(rho_l(i)*AVMW/1000))^(0.5)*X(1);
