function DMDT = Distillation_column_V2_tray55th_controlled(~,S,U)
...
for h=1:61
    ...
    if 1 < h && h ~= 23 && h < 61 && h~=55
        ...
        % V(h-1) = (3 * A_hol * rho_v * ((1/0.145) * ((alfa * 0.5 * (1/14.696))*(101.325) / (AVMW * rho_l(h-1)) - Beta * (dM_hold(h) / (A_act * rho_l(h-1)))))^(1/1.08)) *(0.001/60) ; %kgmol/min to kmol/s
        % I should say that beacause of ignoring vapour hold_up , we
        % calculated vapour amount in bottom and we suppose it to other
        % trays
    ...
    if h ==55
        ...
        U(8) = X(h)*((B_1_ANTOEINE/(A_1_ANTOEINE-log(p(h)/1000))-C_1_ANTOEINE)) + (1 - X(h))*((B_2_ANTOEINE/(A_2_ANTOEINE-log(p(h)/1000))-C_2_ANTOEINE)); % celcuis
    ...
    if h==61
        ...
        rho_v = p(h)/(8.314 * 367*1000); % [kmol/m^3] 
